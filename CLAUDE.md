...
- **Tarjetas regalo**: tablas `gift_cards` (codigo `GC-XXXX-XXXX` unico por tenant, balance, estados active/redeemed/disabled/expired) + ledger `gift_card_transactions` (issue/redeem/assign/refund). Emision `POST /auth/gift-cards`, redencion `POST /auth/gift-cards/redeem {code, booking_id}`: descuenta min(saldo, importe); si cubre todo -> cita `paid` (audit `gift_card_redeemed`), si no -> parcial con `remaining_due_cents` (audit `gift_card_partial`, la cita NO queda paid). UI: Ventas > Tarjetas regalo + boton "🎁 Tarjeta regalo" en detalle de cita. Los canjes (gift y bono) son ATOMICOS: CAS sobre el saldo/`remaining_json` leidos (`rowcount != 1` -> rollback + 409; sin doble gasto concurrente). El codigo se normaliza en servidor (`_normalize_gift_code`: acepta minusculas, sin guiones o sin prefijo GC).
- **Wallet del cliente + canje online (jul 2026, cierra el loop compra→canje)**: (a) **Wallet publica del bono** `GET /bono/{cliente_id}/{wallet_token}` — columnas nuevas `package_purchases.wallet_token` + `initial_json` (snapshot inicial; migracion idempotente con backfill): sesiones restantes por servicio con barras de progreso, caducidad con dias restantes, historial de usos (desde `booking_audit` `package_redeemed`) y CTA "Reservar cita" a la central. El email del bono (venta de mostrador via `_sell_package` Y compra online via webhook — fuente UNICA `_send_package_purchase_email`/`_package_purchase_email_bodies`) lleva el enlace; `_purchase_to_public` expone `wallet_url`/`initial_total`/`used_total`; el portal (Ventas>Bonos) tiene boton "🔗 Wallet" copiable por fila. (b) **Saldo publico de tarjeta** `GET/POST /gift/{cliente_id}/saldo` (`gift_balance_available`: basta con que el tenant haya emitido alguna tarjeta, NO exige el opt-in de venta online; rate limit 10/min por IP contra fuerza bruta; `?code=` prefill desde el email): tarjeta virtual con saldo grande + barra + estado + caducidad; enlazada desde el email de la tarjeta ("Consultar mi saldo") y el footer de `/gift`; `GET /auth/app/gift-cards-public` expone `balance_url`. (c) **Canje online tras reservar en /central**: `POST /central/{id}/redeem-options` y `POST /central/{id}/redeem` (en `public_misc.py`, logica `commerce.booking_redeem_options/booking_redeem_apply`) autorizados por el `manage_token` de la cita (el front lo extrae de `manage_url` de `/agendar`); los bonos se detectan por email/telefono de la reserva (`_packages_for_booking_contact`, normalizacion de telefono via `crm._normalize_phone_for_match`, solo si cubren el `service_id`), la tarjeta exige poseer el codigo. El panel de exito (`showDone`) muestra `#redeemBlock`: tarjeta de bono con "Usar 1 sesion" + toggle "¿Tienes una tarjeta regalo?" con input de codigo; al aplicar oculta el boton de pago (cobertura parcial: informa del resto a abonar en el centro). (d) **Reembolso revierte el activo emitido**: `POST /auth/app/payments/{id}/refund` pasa por `commerce._guard_refundable_asset` (si la tarjeta/bono del pago ya tiene consumo responde 409 pidiendo `force`; app_ui reintenta con `{force:true}` tras confirmDialog); el revert real lo aplica el webhook `charge.refunded` via `commerce._revert_assets_after_refund` — orientado a objetivo con el `amount_refunded` ACUMULADO de Stripe, idempotente: tarjeta queda en `initial - refunded` (ledger `refund`, `disabled` si llega a 0), bono online -> `cancelled` solo con reembolso TOTAL (el parcial es decision del negocio). Tests del viaje completo en `tests/test_bonos_gift_journey.py`.
...
- **Botones de confirmacion WhatsApp**: los recordatorios 24h/2h por WhatsApp llevan botones "✅ Confirmo" / "❌ Cancelar cita" (`bkok_<id>` / `bkcancel_<id>`); el webhook (`_wa_handle_reminder_reply`) verifica que el telefono coincide con el de la cita, registra `attendance_confirmed_by_customer` en audit o cancela la cita. Fallback a texto plano si la API rechaza el interactivo. **Fuera de la ventana de 24 h (sep 2026, `backend/wa_plantillas.py`)** el texto libre no se entrega: el recordatorio sale como plantilla `vantelia_recordatorio_cita` (UTILITY) de la WABA del negocio, con los mismos `bkok_`/`bkcancel_` en sus botones (llegan por el webhook como `type: "button"`). La da de alta y consulta su aprobacion el worker de recordatorios (`wa_plantillas.refrescar_pendientes`); sin plantilla aprobada no sale por WhatsApp y sigue el siguiente canal, con el motivo en `booking_audit` (`reminder_whatsapp_skipped`). Tope opcional `reminders.whatsapp_cap_dia` (Meta cobra cada plantilla al negocio, que necesita metodo de pago en Meta). Las citas `demo_seed` nunca entran en recordatorios ni llamadas. Plan y pendientes en `docs/PLAN_RECORDATORIOS_WHATSAPP.md`.
...
- Modulo `backend/keywords.py` + tabla `keyword_rules` (label, `keywords_json`, reply, `match_mode` any|all, active, position, hits, last_hit_at). `match_reply(cliente_id, message)` devuelve la primera regla activa por `position` que casa.
- **Apagada por defecto**: `config['keyword_rules']['enabled']` (registrado en `clients.CONFIG_EXTRA_SECTIONS`). Sin el flag, `match_reply` sale antes de tocar la DB, asi que los tenants que no la usen no cambian de comportamiento ni pagan consultas.
- Casado literal y explicable: normaliza minusculas/acentos/puntuacion, exige palabra completa, acepta plurales por prefijo (>=4 chars: "masaje" casa "masajes", "spa" NO casa "spandex") y frases multi-palabra por secuencia exacta.
- Canales de TEXTO: chat web (`chat._process_chat_message`, justo despues del menu y antes de disponibilidad/FAQ/RAG, intent `keyword_rule`) y WhatsApp (`whatsapp.py`, tras el saludo puro y solo si `not flow.flow`, para no secuestrar un agendado en curso; registra la conversacion en Conversaciones). La VOZ no las usa (en una llamada no hay texto que casar).
- Endpoints portal: `GET /auth/app/keyword-rules` (devuelve `enabled` + items), `PUT /auth/app/keyword-rules/config` (toggle, manager+), `POST`/`PATCH`/`DELETE /auth/app/keyword-rules[/{id}]`. UI: tarjeta "Respuestas por palabra clave" en la pestana Q&A, **oculta salvo que el tenant tenga la funcion activada o ya tenga reglas**. Contador de usos por regla para medir cuantas preguntas absorbe.
- **En el idioma de quien pregunta (sep 2026, peticion de Cap Rocat):** cada regla puede llevar su respuesta en ingles (`keyword_rules.reply_en`), que sale TAL CUAL a quien escribe en ingles. Otros idiomas (aleman, frances, italiano, holandes) se traducen con el modelo desde la inglesa; si falla, sale la inglesa y si no, la de siempre. El idioma lo decide `keywords.idioma_de` SIN modelo (palabras que delatan cada idioma); lo dudoso («spa?») sale en espanol. Todo en `match_reply`, asi que chat web y WhatsApp lo heredan. Tests: `tests/test_palabras_clave_en_su_idioma.py`.
- Tests: `tests/test_keyword_rules.py`.

**Menu de opciones opt-out (`config['chat_menu']['enabled']`, ago 2026):** negocios que solo quieren respuestas directas (hotel que deriva por telefono) pueden apagar el menu; al saludar responden con `bienvenida` y sin quick actions. Fuente unica `chat._menu_enabled` + `chat._simple_greeting_text`; el chat lo aplica en la rama de saludo y WhatsApp dentro de `_wa_send_main_menu` (asi cubre TODOS los puntos que abren el menu). Sin la seccion en config el menu sigue encendido (comportamiento historico). Endpoints `GET/PUT /auth/app/chat-menu` (manager+) + interruptor en la pestana Tune AI. Registrado en `CONFIG_EXTRA_SECTIONS`.

**Reserva como FORMULARIO dentro de WhatsApp (`backend/wa_flows.py`, ago 2026):** el flujo por mensajes llegaba a 9 interacciones; con WhatsApp Flows el cliente recibe UN mensaje y elige servicio, profesional y hora en una pantalla. Endpoint cifrado `POST /whatsapp/flow` (AES-128-GCM + clave envuelta con RSA-OAEP SHA-256; la respuesta va con el IV INVERTIDO XOR 0xFF; fallo de descifrado = **421**, no 500). El `flow_token` lleva firmado (HMAC) el tenant y el telefono y caduca a las 6h: sin el no se listan datos ni se reserva. Las pantallas NO tienen logica propia (reusan `booking._public_services_for_booking`, `whatsapp._wa_employees_for_service` y los huecos de `agenda`), y la cita la crea el mismo `_wa_create_booking`. Al completarse llega por webhook como `interactive.type = "nfm_reply"`. Apagado por defecto (`WHATSAPP_BOOKING_FLOW_ID` + `WHATSAPP_FLOW_PRIVATE_KEY_B64`); si Meta rechaza el envio, cae solo al flujo por mensajes. La app debe estar suscrita al campo de webhook `flows`. Detalle, estado y trampas en `docs/WHATSAPP_FLOWS_RESERVA.md`.

**Flujo de cita por WhatsApp mas corto (ago 2026):** al cliente ya conocido se le reconoce por su TELEFONO (`crm.contact_by_phone`, verificado por el canal) y no se le vuelven a pedir nombre ni email; el email es opcional; y el paso de notas dejo de ser obligatorio (boton en el resumen). Todo el flujo termina en `_wa_send_booking_summary`, punto UNICO del resumen. Cliente conocido: 5 pasos, todo pulsaciones. Nuevo: 7.

**Alta self-service de WhatsApp + Coexistence (`backend/wa_onboarding.py`, ago 2026):** Meta permite desde mayo 2025 que un numero este a la vez en la app de WhatsApp Business y en la Cloud API (**Coexistence**), asi que el negocio YA NO tiene que sacar su numero del movil. Flujo Embedded Signup: el navegador devuelve `code` + `waba_id` + `phone_number_id` -> `complete_signup` cambia el code por el token PROPIO del negocio, suscribe su WABA a nuestra app y guarda credenciales cifradas (Fernet) en `client_whatsapp_accounts`. Ese token manda sobre el global en `messaging._whatsapp_access_token_for_client`, y su `phone_number_id` entra en `whatsapp._whatsapp_phone_client_map`. **Eco**: cuando su equipo responde desde el movil, Meta manda `message_echoes` -> `_handle_whatsapp_echoes` lo guarda en el historial y hace `inbox.claim`, de modo que el asistente se calla SOLO (nadie pulsa nada). Endpoints `POST/DELETE /auth/app/whatsapp/connect` (owner) + boton en la pestana WhatsApp; el boton no aparece sin `WHATSAPP_APP_ID` + `WHATSAPP_ES_CONFIG_ID` + `WHATSAPP_APP_SECRET`. Desbloqueado el 11-sep-2026: Meta aprobo la revision de la app y esta publicada; el +31 de Vantelia funciona en Coexistence (contesta con el asistente de Vantelia, tenant `metareview`; ver `docs/ESTADO_ACTUAL.md`). El alta self-service sigue limitada por `WHATSAPP_ES_TENANTS`. Lo que Meta entrega sin contenido legible (`unsupported`, audio que no se oye) lo contesta `_wa_mensaje_ilegible` con frase fija: NUNCA pasarle al modelo una instruccion por el hueco del texto de la clienta (queda en el historial como dicha por ella). Detalle y checklist en `docs/WHATSAPP_EMBEDDED_SIGNUP.md`. El alta manual sigue valiendo (numero de demo).

**Intervencion humana sobre una conversacion de WhatsApp (`backend/inbox.py`, ago 2026):** al pasar su numero a Cloud API el negocio pierde la app del movil, asi que necesita poder contestar a mano desde el panel y que el asistente NO hable por encima. Tabla `chat_takeovers` (PK `session_id`): mientras la fila existe y no ha caducado, `inbox.bot_is_muted` es True y `_handle_whatsapp_message` corta ANTES del menu, los flujos y las reglas (solo guarda el mensaje entrante). Caducidad automatica (`INBOX_TAKEOVER_MINUTES`, default 120, contada en minutos) para que nadie deje un chat mudo por olvido; responder renueva el plazo y toma la conversacion sola. Si el equipo contesta desde su app (eco de Coexistence) el plazo es `whatsapp.SILENCIO_TRAS_RESPONDER_MIN` (60; por negocio, `whatsapp.silencio_tras_responder_min`), y al volver un saludo NO saca el menu si ha contestado una persona (`whatsapp._wa_la_ha_atendido_una_persona`): lo coge el agente, cuyo historial no se corta a la media hora cuando hay persona de por medio (`agent.SILENCIO_CON_PERSONA`, 12 h). Tests en `tests/test_vuelve_el_asistente.py`. Ventana de servicio de Meta: `inbox.window_open` (24 h desde el ultimo mensaje del cliente) — fuera de ella se devuelve 409 con explicacion en vez de intentar el envio y fallar (no soportamos plantillas). Endpoints **bajo `/auth/inbox/`**, NO bajo `/auth/conversations/` (la ruta generica `{kind}/{conv_id}` los secuestraria): `GET/POST/DELETE /auth/inbox/{id}/takeover` y `POST /auth/inbox/{id}/reply`. UI: caja de respuesta + boton "Atender yo / Devolver al asistente" en el detalle de Conversaciones (solo canal WhatsApp). Tests en `tests/test_inbox_takeover.py`. **Aviso al negocio (sep 2026):** cuando alguien pide hablar con una persona, `backend/avisos.py` manda un email al negocio (contacto.email, y si no el del dueño) con lo que ha escrito y el enlace al panel. Uno por conversacion cada 30 min, apagable con `config['avisos']['pedir_persona'] = false`, y nunca tumba la conversacion si el email falla. Tests en `tests/test_aviso_al_negocio.py`. PENDIENTE: sigue sin haber aviso de un mensaje normal (solo cuando piden persona), ni push.

**Markdown -> formato WhatsApp:** WhatsApp marca negrita con UN asterisco, asi que el `**negrita**` del modelo se veia con los asteriscos a la vista. `textnorm._markdown_to_whatsapp` (negritas, titulos, vinetas `*`, enlaces `[t](url)`, backticks) se aplica en `messaging._whatsapp_chunks`, el punto UNICO de salida de texto a WhatsApp. Es idempotente: un texto ya en formato WhatsApp no cambia. Tests en `tests/test_chat_menu_y_formato_whatsapp.py`.
...
- **Discovery**: combos sector|city con 2 rondas seguidas sin importables se marcan agotados (`exhausted_targets_json`, se saltan en rotación; reactivables con `clear_exhausted`). Pipeline `new` < 30 → discovery ANTES que cold (el cold sale después con lo importado + pool). Fix: `imported_total` no se incrementaba (el presupuesto de discovery nunca cortaba). **Google Places RETIRADO (04-ago-2026, decisión de coste de Pablo): `GOOGLE_PLACES_API_KEY` vacía a propósito y NO volver a usarla.** El discovery corre gratis con OpenStreetMap/Overpass (`overpass_search` en `scripts/outreach_discover.py`, con `OVERPASS_MIRRORS` + retry porque el endpoint principal da 504/429) + scraping de emails de las webs (también gratis). `discover_companies(source='auto')` sin key = OSM solo, automático; la ausencia de key no es blocker.
...
`IG_DB_PATH`, `IG_GRAPH_TOKEN`, `IG_BUSINESS_ACCOUNT_ID`, `IG_AUTONOMOUS_ENABLED`, `IG_AUTONOMOUS_TICK_MINUTES`, `IG_AUTONOMOUS_DISCOVERY_HOURS`, `IG_RESPECT_WINDOW`, `IG_START_HOUR`, `IG_END_HOUR`, `IG_SKIP_WEEKEND`, `IG_PUBLIC_RATE_LIMIT_SEC`, `IG_REPLIES_INTERVAL_MINUTES`.

### Envio automatico: RETIRADO (9-sep-2026)

Existio un envio automatico de DMs por navegador. Meta restringio por ello una
cuenta de negocio y **se borro el codigo**. No se vuelve a montar: la cuenta de
Meta es de lo que depende el WhatsApp de los clientes. Los DM salen a mano desde
la cola de Drafts del panel, de un clic, por el enlace `ig.me`.
...
## Captacion: nada de automatizar productos de Meta (9-sep-2026)

Habia dos canales que enviaban solos con Playwright y sesion iniciada: **DMs de
Instagram** (45 enviados) y **WhatsApp Web con nuestro propio numero** (211
mensajes). El 9-sep-2026 Meta **restringio una cuenta de negocio** por
"automatizacion que imita la actividad humana". Se retiro el codigo entero: los dos modulos de autosend con Playwright y el de
datos de WhatsApp (en `scripts/`), su modulo de backend, sus endpoints
`/admin/whatsapp/*` e `/admin/instagram/autosend/*`, la pestana WhatsApp del panel
admin y las variables `IG_AUTOSEND_*` / `WA_*`. Si buscas esos ficheros, no
estan: se borraron a proposito, no se traspapelaron.

REGLA: **la cuenta de Meta es el activo del que cuelga el WhatsApp de los
clientes.** Ahorrarse unos DMs no compensa perder el canal por el que trabaja el
producto. La captacion por Instagram sigue viva en modo compliant -discovery,
drafts y envio A MANO de un clic por `ig.me`-, y el email no toca Meta.
...
- Entorno de PRUEBAS en el mismo VPS: `.\deploy\deploy.ps1 -Entorno staging` (`/srv/vantelia-staging`, contenedor `vantelia-staging`, imagen `vantelia-staging:current`, puerto 8001, backups propios). Produccion es el default de todo: sin `-Entorno` nada cambia. El deploy de pruebas se NIEGA a arrancar sin `.env` propio, con `sk_live_` de Stripe o con el mismo `WHATSAPP_ACCESS_TOKEN` que produccion (una prueba escribiendo a una clienta real no tiene vuelta atras). Vigilado por `tests/test_entorno_de_pruebas_aislado.py`; pasos de servidor (DNS, vhost nginx, .env) en `deploy/hostinger/DEPLOY.md`.
