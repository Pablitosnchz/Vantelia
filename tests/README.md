...
| Voz con ElevenLabs y llamadas de Sara | `test_voz_elevenlabs.py`, `test_captacion_voz.py`, `test_lanzador_llamadas.py` — a quién NO se llama y que sin Lista Robinson no suena nadie; `test_cuenta_elevenlabs.py` — si la cuenta se acaba, un correo a Pablo y ninguna llamada; `test_transcripciones_llamadas.py` — cada llamada queda transcrita y clasificada (aviso firmado, recogida en todas las cuentas, exportacion); `test_hablar_con_el_responsable.py` — Sara llega a quien decide (pregunta tras el gancho, espera, rellamada dirigida); `test_lo_que_cazo_astra_24sep.py` — los ocho fallos que reprodujo Astra (Robinson incompleto, revalidar antes de marcar, rotacion atomica, claves censuradas); `test_lo_que_cazo_astra_25sep.py` — sus nueve de la segunda revision (vuelta atras que borra lo creado, rechazo durante Robinson, no llamar antes de la hora dicha, transcripcion a medias, cola que no se atasca) |
