...
Lo que se exige: un error DISTINGUIBLE (`booking.ServicioRetirado`), que diga que
...
def test_el_nucleo_distingue_servicio_retirado_de_hueco_ocupado(client, portal_cookies, catalogo):  # noqa: F811
    ...
    assert not isinstance(ocupado.value, booking.ServicioRetirado), (
        "un hueco ocupado no puede leerse como servicio retirado"
    )

    _retirar(client, portal_cookies, catalogo["retirado"])
    assert booking.servicio_retirado(CID, RETIRADO, source="whatsapp") is True
    assert booking.servicio_retirado(CID, OTRO, source="whatsapp") is False
    assert booking.servicio_retirado(CID, RETIRADO, source="portal_manual") is False

    with pytest.raises(booking.ServicioRetirado) as retirado:
...
def test_voz_traduce_el_rechazo_del_nucleo_y_el_hueco_sigue_siendo_hueco(
    ...
    async def retirado_al_escribir(*_a, **_k):
        raise booking.ServicioRetirado(OTRO)
