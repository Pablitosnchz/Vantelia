...
- **Siguiente:** nada se integra ni se despliega (decision de Pablo, 21-sep): el trabajo se queda en `claude/atencion-whatsapp-20sep` y produccion sigue en `0cb61de`. Lo que falta del bloque: identidad por aviso (negocio, cita, generacion, tipo) para conectar los avisos a la admision de salidas, token de Flow ligado durablemente a la version de atencion, y la voz Realtime con `call_id` del encabezado `Location`.
