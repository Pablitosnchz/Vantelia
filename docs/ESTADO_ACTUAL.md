...
- **Voz nueva con ElevenLabs (decision de Pablo):** comparo a oido OpenAI (gpt-realtime, 2.1, GPT-Live) con voces castellanas nativas y eligio "Laura" de ElevenLabs. Plan y mediciones en `docs/PLAN_VOZ_HUMANA_Y_AUTOCAPTACION.md`. Claves `ELEVENLABS_API_KEY` y `ELEVENLABS_TOOL_SECRET` en el `.env` del VPS (copia previa `/srv/vantelia-backups/env-pre-elevenlabs-20260923`). **24-sep, reserva de cuentas con rotacion (`e535899`, `backend/cuenta_elevenlabs.py`):** Pablo dio varias claves; van en `ELEVENLABS_API_KEYS` (orden de preferencia, por orden suya: primero las Starters, luego la Creator y al final las payg) y la activa es `ELEVENLABS_API_KEY`. Si la activa cae (sin creditos, pago pendiente, plan gratuito, clave rechazada) la voz pasa sola a la primera que funcione, crea alli los agentes y la voz de Laura, borra los de la vieja y avisa por correo; si una preferida vuelve a funcionar, vuelve a ella. La activa se guarda por huella en `storage/elevenlabs_cuenta_activa.json`. Estado al desplegar: activa la Starter (`ccd9db…`, ~35.900 creditos); la Creator (`78cdff…`) y otra Starter (`26f42b…`) tienen factura sin pagar y ElevenLabs les bloquea todo uso (aunque la web diga "Creator"); dos payg (`0b32a4…` 10.000, `9f69c0…` 86). Agentes viejos duplicados borrados; se conserva "Sara - Vantelia (prueba)" de la comparativa. Copias del `.env` en `/srv/vantelia-backups/` (`env-creator-20260924`, `env-antes-reserva-elevenlabs-*`); las que se dejan dentro de `/srv/vantelia/` las borra el deploy.
  - Primera llamada de prueba de un NEGOCIO con Laura (tenant `van`, 24-sep, 92 s, 667 creditos): cita creada de verdad y colgo sola. Arreglado en `c3add0a`: leia mal el codigo de reserva (ahora va deletreado desde la tool, `textnorm.codigo_para_decir`), pedia el telefono a quien llamaba desde el (`{{llamante}}` en el prompt), se presentaba dos veces y la voz variaba (estabilidad 0,7 / parecido 0,9, comun a Sara). Ningun negocio real tiene aun `voice.engine = "elevenlabs"`.
  - **Modelo de chat y WhatsApp = el de Laura (`gpt-4.1-mini`) desde el 25-sep (`d6d8bbf`, `CHAT_MODEL` en el VPS).** Medido antes con el banco sobre foto de produccion: 45/45 con los dos modelos, sin criticos.
  - Incidente 24-sep 16:14-16:16 UTC: la web dio 502 (12 peticiones) al recrear la app. El proxy (`nginx-proxy-manager`, recreado por otro a las 15:25) busca `vantelia-app` por nombre y solo lo tenia conectado a mano. Arreglado en el compose (`3c66de5`, red externa `nginx-proxy-manager_default`; test `test_la_web_llega_a_la_app.py`).
  ...
  - `backend/captacion_voz.py` (`7163769`, `f6d5f4d`): Sara llama a negocios con guion legal (IA + comercial + opcion de no mas llamadas, corto y tras el gancho por peticion de Pablo). Tool `enviar_informacion` (SMS a moviles, email del negocio a fijos; avisa a Pablo), `volver_a_llamar`, `no_volver_a_llamar` (tabla `no_llamar`, se mira ANTES de marcar). Contestador/buzon: lo detecta ElevenLabs (`voicemail_detection` sin mensaje), NO Twilio, cuya deteccion retrasaba ~6 s el saludo. Cierre (24-sep, tras 3 llamadas de prueba a Pablo): SMS a moviles, email del negocio (de la base de captacion) a fijos, y solo si no hay ninguno se pide email sin deletrear; la demo del negocio se empieza a generar en ese momento; modelo `gpt-4.1-mini` (Gemini metia ingles y se quedo mudo). Probado: SMS entregado, cuelga sola, saludo al descolgar. Tabla `llamadas_voz` en la base de captacion. Primera llamada real a Pablo el 24-sep: bien; apertura acortada despues.
  - `backend/lanzador_llamadas.py` + `backend/lista_robinson.py` (`1c4e6a7`, desplegado APAGADO): Sara llama sola a fijos de la base de captacion, lunes a viernes 10:00-12:30 y 16:00-18:00 (el lunes lo anadio Pablo el 24-sep), tras consultar la Lista Robinson (sin respuesta no hay llamada). Vista "Llamadas" en el panel admin con transcripcion. Bloqueos que quedan en produccion: `CAPTACION_LLAMADAS_ENABLED`, credenciales de Robinson y numero espanol `CAPTACION_TWILIO_NUMBER`; ademas el interruptor del panel.
  - **Desplegado el 25-sep tras el OK de Astra (decision de Pablo: no desplegar sin su revision)** -> rama `claude/transcripciones`: `3a33f02` (los 8 fallos que cazo Astra en el lanzador y la rotacion), `5e2c841` (Sara llega a quien decide: pregunta tras el gancho, espera callada, rellamada dirigida; `docs/PLAN_HABLAR_CON_EL_RESPONSABLE.md`), `246bdce` (transcripciones de cada llamada + exportacion jsonl; fase 1 de `docs/PLAN_SEGUNDA_OPORTUNIDAD_LLAMADAS.md`) y cuatro vueltas de arreglos de sus revisiones (`08df105`, `c40a60c`, `5dcc0ad`, `b69295c`; actas `docs/REVISION_SARA_*.md`, casos en `tests/test_lo_que_cazo_astra_25sep.py`). Lo que cambia de fondo: la rellamada lee la hora como tramo (desde/hasta, "no esta hasta las cinco" = desde las cinco) y ante cualquier duda o negacion NO rellama; el intento de recoger una transcripcion se apunta antes de pedirla y hay 55 min entre intentos. Suite completa 3605 verde; OK de Astra en `cb69c90`. Nota: `tests/test_voice_engine.py` falla en una copia de trabajo recien creada (sin `storage/vantelia.db`): fallo previo del entorno.
  - **Lista Robinson ACTIVA (25-sep):** credenciales `ROBINSON_API_KEY/SECRET` en el `.env` local y del VPS (entran al recrear el contenedor); API probada en vivo, firma aceptada.
  - Pendiente: **numero 91** -> `CAPTACION_TWILIO_NUMBER`. **26-sep: bundle "Spain Local - Business" (`BU27565de0…`) ENVIADO a revision** (`pending-review`, evaluacion automatica `compliant`): titular "SANCHEZ SANCHEZ PABLO" tal cual en el certificado de situacion censal de la AEAT (unico documento: identidad, NIF y domicilio fiscal en Torrejon de Ardoz, valido para el 91), direccion `AD6a778e…` igual que el certificado, avisos a vanteliadigital@gmail.com. El "Individual" de junio se rechazo por nombre incompleto ("Pablo Sanchez"), DNI sin reverso y sin direccion visible. Al aprobarse: comprar el 91 con ese bundle, `CAPTACION_TWILIO_NUMBER` en el VPS e importarlo a ElevenLabs. `CAPTACION_LLAMADAS_ENABLED` sin poner (necesita orden explicita de Pablo). Remitente SMS alfanumerico "Vantelia"; transcripciones de ElevenLabs a Conversaciones (encargo a Astra); widget web con ElevenLabs.
...
- **Regla que fija la fase 3, porque no estaba escrita:** la IA no sostiene conversaciones con la atencion pausada, la arranque quien la arranque. Por eso la llamada de confirmacion se frena TAMBIEN desde el boton del panel (409 con el motivo). Lo que el equipo escribe o manda a mano sigue saliendo, y lo que solo deja constancia de algo que ya paso (status callback de Twilio, transcripcion al colgar) no se toca. Confirmado por Pablo el 21-sep: se queda asi.
- **Fase 4 hecha:** interruptor de pausa en el panel del negocio (tarjeta en Cuenta + aviso permanente fuera de ella), permiso `channels.manage`, CAS con 409 `ATTENTION_STALE` para dos pestanas abiertas, y lectura administrativa `GET /admin/attention` que NO puede cambiar el estado de nadie. Suite completa del arbol congelado: 3320 passed, 1 skipped, sin fallos, 2251.58s.
- **Fase 5 cerrada con alcance declarado (decision de Pablo, 21-sep):** solo instrumentos sin coste. Suite 3320 passed/1 skipped, 22 mutaciones causales rojas, `qa_e2e` 94/94 con salida 0 y 16 pruebas Node del widget. SIN banco de casos, SIN simulador y SIN humo: gastan modelo y el humo es justo el que caza los fallos de CAMINO que la suite no ve. Sin proveedores reales. Detalle y limites en MEDICION_CIERRE_COMPARABLE.md.
- **De paso:** `scripts/qa_e2e.py` daba un BUG falso y salia con codigo 1 desde hacia tiempo (comprobado igual en `0cb61de` desplegado y en `f11132c`); era el arnes, no el producto. Arreglado en `34dab9f`, y ahora acredita que la reserva por WhatsApp crea la cita.
- **21-sep, tarde: identidad por aviso y worker conectado.** Rama nueva `claude/avisos-identidad-21sep` (E:/vp-atencion-wa) sobre `5be4463`. `a3df1ee` es el trabajo de Astra rescatado sin cambios de su copia sin commitear (astra/avisos-identidad-19sep; su copia no se toco, parche con `git diff --cached`). `ec3a58d` conecta los recordatorios del worker: turno por aviso, vigencia 900 s (un ticket vencido cuenta como supresion y con turno es terminal), una supresion no cuenta como fallo. `c9bc7ff` reapunta dos vigilantes de reglas que leian una funcion que el commit de Astra partio en dos. Suite completa: 3342 passed, 1 skipped, 2 failed (esos dos vigilantes, ya arreglados y comprobados con mutacion; no se relanza la completa por un cambio solo de pruebas). `qa_e2e` 94/94, salida 0. Mutaciones: 4 del worker en rojo.
- **Formulario de WhatsApp ligado a la version (`0657fa5`):** el token del Flow lleva firmada la version de atencion; uno abierto antes de una pausa no revive al reactivar, ni en las pantallas ni en el envio final. No poder comprobarlo no se le presenta a la clienta como "caducado". Suite completa del arbol final: 3348 passed, 1 skipped, 0 failed, 2456.33s; `qa_e2e` 94/94 salida 0.
- **La captacion ya avanza por la ciudad (`c2ec1c8`, DESPLEGADO el 22-sep a las 23:26 Europe/Madrid, humo 5/5, suite 3391/0):** el 22-sep se quedo otra vez sin cantera (5 prospects, 26 objetivos agotados con Madrid sin tocar) porque Overpass devuelve siempre los mismos primeros resultados y a las 2 rondas el objetivo se daba por agotado. Ahora la busqueda recuerda que negocios miro en cada sector y ciudad (tabla `osm_vistos`), los salta, pide una ventana mayor, y un objetivo solo se agota si NO avanzo. Comprobado en produccion contra el mapa real: segunda ronda sin repetir ninguno. Objetivos: 65 (3 sectores x 16 ciudades de Madrid y alrededores; se anadio estetica, que en OSM es otro tipo y es cantera nueva) y marcas de agotado borradas; copia `storage/outreach/outreach-pre-objetivos2-20260922.db`. OJO: fuera de 9-19 la ronda entera se salta, asi que la primera busqueda con esto es manana a las 9:00.
- **Leads descartados por Pablo (22-sep):** MG Clinic y Fisionervion pasan a `lost`; 0 respuestas sin atender.
- **Captacion parada del 12-ago al 21-sep sin avisar (diagnosticado el 21-sep):** piloto encendido, sin pausa y con SMTP dedicado bien, pero sin cantera (385 contactados, 9 de 10 objetivos agotados desde agosto) y con el unico objetivo vivo, «centro de masajes · Madrid», fallando cada hora porque el buscador gratuito no conocia el sector y el bucle no anotaba el fallo. Arreglo en `fbf2d8b` (`f122e4c`, DESPLEGADO el 21-sep a las 21:12 Europe/Madrid por orden de Pablo: VERSION.json `f122e4c` sin cambios sucios, humo 5/5; suite previa 3359 passed / 0 failed): el buscador reconoce masajes, un fallo cuenta como ronda vacia y el panel avisa cuando no queda a quien escribir o el buscador falla. DECISION DE PABLO: escribir a peluquerias y barberias de Madrid y alrededores; aplicado en produccion el 21-sep (26 objetivos: los 10 de antes + 16 nuevos), con copia previa en `storage/outreach/outreach-pre-peluquerias-20260921.db`. Ojo: peluqueria y barberia son el MISMO tipo en OpenStreetMap (`shop=hairdresser`), asi que son 8 objetivos efectivos. Prueba en seco: de 60 peluquerias de Madrid, 26 con web y 10 con email publico. Los envios se reanudan el 22-sep a las 9:00, con el calentamiento (10/dia al principio).
- **Alicia pasa a Pro (decision de Pablo, 22-sep):** su suscripcion (tabla `subscriptions`, que manda sobre config.json) estaba en Business; ahora Pro, 5.000 mensajes, sin voz ni SMS, WhatsApp si. Cambiada con `db_set_subscription_from_stripe` (el camino del webhook de Stripe), conservando periodo y uso (332). Antes: business, cupo 25000, fila `aff95b94d20a64ff3c11d385`. OJO: `config.json` sigue diciendo business, pero la BD manda. Antes de pasarla se cambio la regla del tope para TODOS (`140ecc2`, desplegado, suite 3386/0, humo 5/5): las 500 citas/mes de Pro cuentan solo lo que coge el asistente y el alta manual del panel nunca se bloquea por tope; con la regla vieja su equipo podia quedarse sin apuntar citas a mitad de mes.
- **Textos de captacion POR SECTOR (22-sep, DESPLEGADO `1899cfe` a las 11:35 Europe/Madrid por orden de Pablo):** el 22-sep salieron 7 correos a centros de masajes con el texto de peluquerias. Ahora cada negocio recibe el de su sector: frases por sector en `outreach_templates.SECTOR_COPY` (peluqueria, barberia, masajes, estetica, fisio, dental, veterinaria, clinica, y una version general), reconocido por `prospects.niche`. Lote `2026-09-por-sector-v1` en el codigo, aplicado solo al arrancar (comprobado: las 4 etapas en esa version, historial con foto de las anteriores). Solo a peluquerias y barberias se les dice que se trabaja con una peluqueria. Suite 3381 passed + 2 pruebas que fijaban el literal «Responde» actualizadas; humo 5/5. Pruebas de fisio y masajes enviadas a Pablo por el camino real.
- **Textos de captacion nuevos (aprobados por Pablo el 21-sep):** lote `2026-09-peluquerias-humano-v1` en las plantillas del panel (cold A/B, fu1, fu2, cierre), escritos para peluquerias: sin «Responde 1 = si / 2 = no», el nombre del negocio una sola vez y WhatsApp en vez de «consultas de la web». A y B rellenas en todas las etapas (una B vacia devolvia esa mitad a los textos antiguos del codigo). Copia de la base en `storage/outreach/outreach-pre-textos-20260921.db` y foto de las plantillas anteriores en `outreach_template_bundle_history` para volver atras. El pie legal NO se toca (decision de Pablo). Ojo: el mismo tipo de OpenStreetMap trae barberias, y el texto habla de «clientas».
- **Cap Rocat:** sus 7 reglas ES/EN del Excel ya estaban cargadas y en uso (32 respuestas dadas); no hay nada que adelantar hasta que devuelvan su Excel revisado.
- **Hueco cerrado despues del despliegue (`b412b94`, DESPLEGADO el 21-sep a las 15:20 Europe/Madrid por orden de Pablo: VERSION.json `b412b94` sin cambios sucios, health OK, humo 5/5; suite previa 3353 passed / 0 failed, `qa_e2e` 94/94):** una llamada de telefono ya en curso al pulsar la pausa podia seguir creando, cancelando o moviendo citas, porque el puente solo miraba la pausa al conectar. Ahora cada herramienta de la llamada lleva su turno y, si la atencion esta pausada, no se ejecuta y la llamada se cierra con la despedida. La voz del widget, que solo tenia una foto previa, lleva tambien turno. Queda abierto: una conversacion de voz del NAVEGADOR ya abierta sigue hablando (no puede tocar la agenda); cortarla exige el `call_id` de Realtime y llamadas reales.
- **DESPLEGADO en produccion el 21-sep a las 12:38 Europe/Madrid, por orden de Pablo:** `main` = `716a4b4` (fast-forward, subido), VERSION.json `716a4b4` sin cambios sucios, health OK, humo 5/5. Comprobado en la BD viva: tablas de atencion creadas, indice `idx_attention_notice_attempt` presente y NINGUN negocio en pausa (todos quedan activos por defecto: sin fila = activa). Copia previa de la BD en /srv/vantelia-backups/pre-deploy-*.db. Pendiente del bloque: SOLO la voz Realtime con `call_id` del encabezado `Location`, que necesita llamadas reales a OpenAI. (El token de Flow ya quedo ligado a la version.)
- **Deuda anotada en el codigo, no tapada:** resenas, rebooking y avisos de caducidad siguen solo con la puerta previa, sin turno ni identidad de aviso; su sello propio evita repetirlos. El ticket del worker nace con `event_at` = ahora, no con el nacimiento del aviso, y es a proposito: decision de Pablo del 21-sep, el recordatorio cuya ventana coincide con la reactivacion SALE. Por eso NO se integra la migracion de astra/recordatorio-origen-19sep (detalle en PAUSA_TEMPORADA_DISENO.md). El formulario de Flows responde con pantalla de aviso, no con el HTTP 427 del ejemplo de Meta para tokens invalidos: cambiar lo que recibe Meta no se puede comprobar sin su entorno.
