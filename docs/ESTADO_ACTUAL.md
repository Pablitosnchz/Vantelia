...
- **Siguiente (Claude):** tras el despliegue, seis arreglos más en `claude/candidato` (pack con `preferir_packs`, fechas en humano, freno de «cerrado», oferta retirada, reloj de la ventana de reserva, «qué me recomiendas», cita confirmada que sí existe; HEAD 36b4d8d, todos con test rojo). En curso: suite completa y medición con modelo real; luego resultados a Pablo y propuesta de despliegue. Pendiente: vigilar conversaciones reales de Alicia (mañana o pasado), `exigir_dos_apellidos` sin activar en producción (pregunta a Pablo), plantilla de recordatorio de Meta, fase 4 del plan con el inventario de frenos.
