...
- **Tarea:** candidato 0bca1eb **desplegado en producción** el 14-sep a las 01:20 (VERSION.json 109b091) por orden de Pablo; vigilar cómo funciona con clientas reales y cerrar los pendientes. Astra sin créditos hasta el 13-oct.
...
- **Siguiente (Claude):** candidato **bec310f** en `claude/candidato` (producción sigue en 0bca1eb): siete arreglos tras el despliegue más el test de concurrencia rehecho, todos con test rojo. Medido ed94be1 con modelo real: banco Alicia 43/43, crítico 6/6 sin fechas ISO, humo 5/5, portal 6/6, suite 2675 verde. Sobre bec310f: metareview 15/16 (manicura arreglada), humo 5/5, suite 2678 verde. Pendiente: orden de despliegue de Pablo. Abierto: `exigir_dos_apellidos` sin activar en producción (pregunta a Pablo); WhatsApp de Alicia sin conectar (bloquea recordatorios); vigilar conversaciones reales tras la instalación; `horario-escrito-manda` depende del día (instrumento); fase 4 con el inventario de frenos.
