...
- **Siguiente:** nada se integra ni se despliega (decision de Pablo, 21-sep); produccion sigue en `0cb61de`. Pendiente del bloque: SOLO la voz Realtime con `call_id` del encabezado `Location`, que necesita llamadas reales a OpenAI. (El token de Flow ya quedo ligado a la version.)
