...
- **Siguiente:** nada se integra ni se despliega (decision de Pablo, 21-sep); produccion sigue en `0cb61de`. Pendiente del bloque: el ORIGEN de cada recordatorio (Astra tiene otra copia sin commitear, astra/recordatorio-origen-19sep), el token de Flow ligado a la version de atencion, y la voz Realtime con `call_id` del `Location` (necesita proveedor real).
- **Deuda anotada en el codigo, no tapada:** resenas, rebooking y avisos de caducidad siguen solo con la puerta previa, sin turno ni identidad de aviso; su sello propio evita repetirlos. El ticket del worker nace con `event_at` = ahora, no con el nacimiento del aviso: una reactivacion dentro de la banda del recordatorio lo deja salir. En Flows, la puerta impide ofrecer catalogo y huecos, pero el token no esta ligado a la version de atencion.
