...
- **Siguiente (Claude):** candidato final **0bca1eb** en `claude/candidato`. Dos revisiones antes de desplegar (independiente en frío y Astra) dieron 6 hallazgos, y leyendo conversaciones salió un freno que negaba horas libres: todo arreglado con test rojo sin el arreglo; Astra da OK a 0bca1eb. Medido con modelo real sobre 0bca1eb: crítico 6/6 al primer intento, metareview 16/16, humo 5/5, portal y reinicios 6/6 (uno al segundo intento). Después: contar a Pablo lo serio (incluido un riesgo ya en producción: mechas en 75 min en vez del pack con `preferir_packs`) y pedir la orden de despliegue; tras desplegar, crear la regla de orientación de Alicia en producción y verificarla.
