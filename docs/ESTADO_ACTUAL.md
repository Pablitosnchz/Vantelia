...
- **Siguiente (Claude):** dos revisiones antes de desplegar dieron cambios: la de Astra (a329fe0, «Pack» en mayúsculas contaba como cambio de servicio) y una independiente en frío (2a794c2: la hora tomaba el número del día, `booking_name` guardaba conversación como nombre, «sí, el jueves a las 17» perdía día y hora, el freno de cancelar bloqueaba cancelaciones legítimas). Todo con test rojo sin el arreglo. Re-medición con modelo real en curso sobre 2a794c2 (banco Alicia, crítico ×6, humo, portal/reinicio, metareview, suite). Después: informe, contar a Pablo lo serio y desplegar; tras desplegar, crear la regla de orientación de Alicia en producción y verificarla.
