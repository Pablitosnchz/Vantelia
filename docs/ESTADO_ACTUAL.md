...
- **Voz nueva con ElevenLabs (decision de Pablo):** comparo a oido OpenAI (gpt-realtime, 2.1, GPT-Live) con voces castellanas nativas y eligio "Laura" de ElevenLabs. Plan y mediciones en `docs/PLAN_VOZ_HUMANA_Y_AUTOCAPTACION.md`. Claves `ELEVENLABS_API_KEY` y `ELEVENLABS_TOOL_SECRET` en el `.env` del VPS (copia previa `/srv/vantelia-backups/env-pre-elevenlabs-20260923`). **24-sep: produccion pasa a una cuenta Creator NUEVA** (otra cuenta, no la Starter mejorada): voz Laura anadida a esa cuenta con el mismo id, agentes de Sara y `van` creados de nuevo en ella (`1c4e6a7` hace que sincronizar cree el agente si el id guardado no existe en la cuenta). Copia del `.env` anterior en `/srv/vantelia/.env.bak-elevenlabs-*`. La cuenta Starter vieja queda sin uso (Pablo la rota o la borra). OJO: la suscripcion Creator salia `past_due` al cambiar; revisar la facturacion.
