...
| `backend/captacion_voz.py` | Captacion por telefono: la agente Sara (ElevenLabs) llama a los negocios con el guion legal (quien llama, que es una IA, que es comercial y que puede pedir no mas llamadas). Tool `enviar_informacion` (SMS si es movil, email del negocio si es fijo y lo sabemos, si no lo pide; apunta el interes y avisa a Pablo), `volver_a_llamar` y `no_volver_a_llamar` (ese telefono no vuelve a sonar); contestador = colgar sin mensaje. Llamadas en `llamadas_voz` de la base de captacion (con `origen` manual/auto y el `conversation_id` de ElevenLabs para leer la transcripcion). Quien decide (docs/PLAN_HABLAR_CON_EL_RESPONSABLE.md): tras el gancho pregunta UNA vez si habla con quien lleva el negocio; tool `anotar_responsable` (interlocutor, nombre, cuando, email; un movil de un empleado solo queda como nota); si se lo pasan, espera callada (`skip_turn`) y se vuelve a presentar entera; el saludo pregunta por `{{a_quien}}` (negocio o, en una rellamada dirigida, la persona). ElevenLabs clasifica cada llamada al terminar (`DATOS_AL_TERMINAR`: interlocutor, desenlace, escucho_la_demo, responsable_nombre). Prueba: `POST /admin/captacion/voz/llamada-prueba`; las de captacion las hace `lanzador_llamadas`. |
| `backend/lanzador_llamadas.py` | Lanzador automatico de las llamadas de Sara. Tres llaves: `CAPTACION_LLAMADAS_ENABLED=true`, interruptor del panel (tabla `llamadas_config`, apagado de serie) y nada en `bloqueos()` (Robinson, numero espanol `CAPTACION_TWILIO_NUMBER`, Twilio, ElevenLabs, agente creada). Solo fijos (primero los que abrieron o pincharon nuestros correos), Lista Robinson antes de marcar (sin respuesta no hay llamada; cache 30 dias en `robinson_consultas`), lunes a viernes 10:00-12:30 y 16:00-18:00 Madrid, max 2 intentos si nadie lo cogio, nada a quien recibio correo hace <3 dias, una llamada cada vez con hueco y cupo diario. Hilo cada 5 min. Rellamada dirigida (`rellamadas_dirigidas`): si cogio alguien del equipo y dio el nombre de quien decide, UNA rellamada al fijo del negocio en la franja y el dia que dijeron (`cuando_esta`), antes que las llamadas en frio y con Robinson; nunca si hubo no llamar, interesado o la transcripcion dice rechazo. Si ElevenLabs no conecta una llamada ya descolgada (`fallo_de_voz`, desde la ruta `/voice/el-captacion/twiml`), el lanzador se apaga solo y avisa a Pablo por correo. Panel: vista "Llamadas" del admin (`GET /admin/captacion/llamadas`, `PUT .../config`, `POST .../ronda`, `GET .../{id}/transcripcion`). |
| `backend/transcripciones_llamadas.py` | Transcripciones de las llamadas de Sara para analizarlas (Pablo, 25-sep-2026): tabla `llamadas_transcripcion` (turnos con herramientas, resumen, duracion y lo que ElevenLabs clasifica al terminar). Llegan por el aviso de fin de llamada firmado (`POST /voice/el-captacion/fin`, `ELEVENLABS_WEBHOOK_SECRET`, cabecera `ElevenLabs-Signature`; hay que darlo de alta en cada cuenta) o por `recoger_pendientes` (cada hora desde el vigilante de la cuenta), que prueba TODAS las claves de la reserva. Lo clasificado rellena interlocutor/responsable si Sara no los apunto y un `rechazo` impide la rellamada dirigida. Panel: `leer` (guardada o la pide y guarda); exportacion `GET /admin/captacion/llamadas/transcripciones.jsonl`. |
...
| `backend/cuenta_elevenlabs.py` | Las cuentas de ElevenLabs. `estado()` (cache 30 min por cuenta) dice si una sirve: pago pendiente con factura abierta (ElevenLabs bloquea todo uso aunque diga `creator`), plan gratuito, creditos agotados o clave rechazada = no. Reserva `ELEVENLABS_API_KEYS` en orden de preferencia (Pablo, 24-sep-2026: "usa la creator primero"): si la activa cae, `rotar()` pasa a la primera que sirva, crea alli los agentes (`sincronizar_agentes`, voz de Laura incluida), borra los de la vieja y avisa por correo; y si una preferida vuelve a funcionar, se vuelve a ella. La activa se guarda por huella (nunca la clave) en `storage/elevenlabs_cuenta_activa.json` (`aplicar_activa_guardada` al arrancar). El lanzador la asegura antes de cada ronda y tras un fallo de voz con alguien al telefono. Hilo cada hora; sin ninguna cuenta que sirva, UN correo por problema cada 24 h y Sara no llama. Forzar cambio: `POST /admin/voz/cuenta/rotar`. |
| `backend/voz_elevenlabs.py` | Voz con ElevenLabs Agents (voz castellana "Laura" y modelo de turnos de ElevenLabs). Un agente por negocio generado desde las MISMAS fuentes que la voz de OpenAI (instrucciones, saludo con aviso de IA y tools de cita); ElevenLabs llama a `POST /voice/el/{cliente_id}/tool/{nombre}`, que ejecuta `voice._voice_dispatch_tool`. Sincronizacion: `POST /admin/clientes/{cliente_id}/voz-elevenlabs` (si el id guardado no existe en la cuenta de la clave actual, crea el agente de nuevo). Las transcripciones las guarda `transcripciones_llamadas`. |
