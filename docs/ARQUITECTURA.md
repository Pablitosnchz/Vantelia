...
| `backend/captacion_voz.py` | Captacion por telefono: la agente Sara (ElevenLabs) llama a los negocios con el guion legal (quien llama, que es una IA, que es comercial y que puede pedir no mas llamadas). Tools `apuntar_interes` (avisa a Pablo), `volver_a_llamar` y `no_volver_a_llamar` (ese telefono no vuelve a sonar); contestador = colgar sin mensaje. Llamadas en `llamadas_voz` de la base de captacion. Prueba: `POST /admin/captacion/voz/llamada-prueba`. Sin lanzador automatico todavia. |
