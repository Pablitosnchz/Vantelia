...
| `backend/atencion_operaciones.py` | Tickets por evento y admisiones por canal/fragmento, persistidos bajo la transacción de la autoridad de atención. Solo el primer ganador recibe permiso de red; duplicados, supresiones y resultados inciertos no lo renuevan. Conserva el resultado con propietario; no decide reintentos, no sustituye `notice_deliveries` ni conecta canales. Consultar no reclama workers ni recupera tokens perdidos para reconciliación automática. |
